...
class MapSubscriber(Node):
    def __init__(self):
        ...
        self.tree_edges_pub = self.create_publisher(
            TreeEdges, '/spanning_tree', 10)
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        tree_msg = TreeEdges()
        for a, b in tree_edges:
            start_pt = Point(x=float(a[0]), y=float(a[1]), z=0.0)
            end_pt = Point(x=float(b[0]), y=float(b[1]), z=0.0)
            tree_msg.start_points.append(start_pt)
            tree_msg.end_points.append(end_pt)

        self.tree_edges_pub.publish(tree_msg)

        # Path planning

        cur = (0.0, 0.0) # Make it - the blue node closest to robot pose at T = 0
        sub = ((cur[0] - self.cell_size / 4), (cur[1] - self.cell_size / 4))
        start = sub
        self.path = [sub]
        goal = (0.0, 0.0)
        dist = float('inf')
        ...
        while (goal != start):
            ...
            for x, y in nb:
                ...
                next_subs[node].append(subnodes)

            for key in next_subs:
                d = math.hypot(sub, self.get_closest_subnode(sub, self.get_subnodes(next_subs[key])))
                if d < dist:
                    dist = d
                    goal = next_subs(key)
                    goal_key = key
            ...
            # Handle leaf nodes                
            if len(adjacency[cur]) == 1:
                if (sub[0] < cur[0] and sub[1] > cur[1]):
                    self.path.append(sub[0] + self.cell_size / 4, sub[1])
                    self.path.append(sub[0], sub[1] - self.cell_size / 4)

                elif (sub[0] > cur[0] and sub[1] > cur[1]):
                    self.path.append(sub[0], sub[1] - self.cell_size / 4)
                    self.path.append(sub[0] - self.cell_size / 4, sub[1])
        ...
        self.path.clear()
        self.get_logger().info('Path planning completed and published.')
    ...
    def get_neighbours(self, node) -> list:
        ...
        if not neighbours:
            ...
            self.get_logger().info(f'Found {len(neighbours)} neighbours for node {node}')
        
        neighbours = [(round(x, 3), round(y, 3)) for x, y in neighbours]
        return neighbours
    
    def get_subnodes(self, node) -> list:
        sub_nodes = []
        for dx, dy in [(self.cell_size / 4, 0), (-self.cell_size / 4, 0), (0, self.cell_size / 4), (0, -self.cell_size / 4)]:
            sub_node = (round(node[0] + dx, 3), round(node[1] + dy, 3))
            sub_nodes.append(sub_node)
        return sub_nodes
    
    def get_closest_subnode(self, sub, nb_subs) -> tuple:
        closest_subnode = ()
        min_dist = float('inf')
        for subnode in nb_subs:
            ...
            if dist < min_dist:
                min_dist = dist
                closest_subnode = subnode
        if not closest_subnode:
            self.get_logger().warn(f'No closest subnode found for {sub}')
        else:
            self.get_logger().info(f'Closest subnode to {sub} is {closest_subnode}')
        return closest_subnode
