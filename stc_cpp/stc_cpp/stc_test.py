...
from stc_cpp.obj_detection import ObjectInspector
...
class KruskalSTCNode(Node):
    def __init__(self):
        ...
        self.inspected_major_cells = set()

        self.subcell_visit_count = 0
    ...
    def result_callback(self, future, idx):
        ...
        self.get_logger().info(
            f"Reached waypoint {idx+1}"
        )

        self.subcell_visit_count += 1

        # Run inspection every other subcell
        if self.subcell_visit_count % 2 == 0:

            self.get_logger().info(
                "Running object inspection..."
            )
